...
public class ArduPilotDriver : IAutopilotDriver
{
    ...
    private readonly IParameterUploadStrategy uploadStrategy;
    ...
    public ArduPilotDriver(
        ...
        IParameterUploadStrategy uploadStrategy,
    ...
    CancellationToken ct)
    {
        ...
        var context = new ParameterUploadContext(telemetry, port, ReconnectAfterBootAsync);

        var result = await uploadStrategy.UploadAsync(context, parameters, progress, ct);
    ...
    {
        _telemetryGate.Wait();
        try { StopTelemetryCore(); }
        finally { _ = _telemetryGate.Release(); }
    }
    ...
    {
        await _telemetryGate.WaitAsync();
        try { StopTelemetryCore(); }
        finally { _ = _telemetryGate.Release(); }
    }
    ...
    /// Stops the telemetry background task and cancels it. Waits for the task to complete.
    /// </summary>
    private void StopTelemetryCore()
    {
