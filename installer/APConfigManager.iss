...
SetupIconFile=..\src\APConfigManager.Desktop\APConfigManager.Desktop\Assets\app.ico
...
[Files]
; Desktop app
Source: "..\build\publish\desktop\*"; DestDir: "{app}\desktop"; Flags: ignoreversion recursesubdirs createallsubdirs
; API + React UI
Source: "..\build\publish\api\*"; DestDir: "{app}\api"; Flags: ignoreversion recursesubdirs createallsubdirs
; Windows App SDK + .NET runtimes (kept next to this .iss, in installer\)
...
[Run]
; Install runtimes silently (needs admin)
...
Filename: "{tmp}\aspnetcore-runtime-8.0.28-win-x64.exe"; Parameters: "/install /quiet /norestart"; StatusMsg: "Installing ASP.NET Core Runtime..."; Flags: waituntilterminated runhidden
; Launch app after install
Filename: "{app}\desktop\{#MyAppExeName}"; Description: "Launch AP Configuration Manager"; Flags: nowait postinstall skipifsilent

[UninstallDelete]
; Clean up app folder (but NOT %LocalAppData%\APConfigManager — user data)
